...
def pretreatment(test_image):
    ...
    cv.imwrite('gaussianblur1.jpg', gray_gaussianblur)
    ...
    thresh, result = cv.threshold(gray_gaussianblur, 110, 255, cv.THRESH_BINARY)
    cv.imwrite('binary1.jpg', result)
    res_inv = cv.bitwise_not(result)
    cv.imwrite('res_inv1.jpg', res_inv)
...
if __name__ == '__main__':
    ...
    # # 根据确定的位置分割字符
    # print(len(pos))
    rec_image = cv.imread('rec_image1.jpg')
    for m in range(-18, 0,1):
